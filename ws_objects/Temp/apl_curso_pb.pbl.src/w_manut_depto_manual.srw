$PBExportHeader$w_manut_depto_manual.srw
$PBExportComments$Manutenção de departamento
forward
global type w_manut_depto_manual from window
end type
type cb_exportar from commandbutton within w_manut_depto_manual
end type
type cb_inserir from commandbutton within w_manut_depto_manual
end type
type cb_excluir from commandbutton within w_manut_depto_manual
end type
type cb_adicionar from commandbutton within w_manut_depto_manual
end type
type cb_gravar from commandbutton within w_manut_depto_manual
end type
type cb_recuperar from commandbutton within w_manut_depto_manual
end type
type dw_manut_depto from datawindow within w_manut_depto_manual
end type
end forward
...
dw_manut_depto dw_manut_depto
end type
global w_manut_depto_manual w_manut_depto_manual
...
// inicialização
vlb_Ok = true
vli_NumLinhas = dw_Manut_Depto.rowCount()


// validação de campos
for i = 1 to vli_NumLinhas
	// valida código
	vli_CodCargo = dw_Manut_Depto.getItemNumber (i, 'cod_cargo')
	...
	// valida descrição
	vls_DescCargo = dw_Manut_Depto.getItemString (i, 'desc_cargo')
	...
	// valida salário
	vldc_Salario = dw_Manut_Depto.getItemNumber (i, 'salario')
...
// inicialização
vlb_Ok = true
vll_QtdLinhas = dw_Manut_Depto.rowCount()


// inclui informações obrigatório
for i = 1 to vll_QtdLinhas
	dw_Manut_Depto.setItem (i, 'id_aluno', 2)
next
...
event u_adicionalinha();dw_Manut_Depto.insertRow (0)
...
vll_NumLinhaAtual = dw_Manut_Depto.getRow()

// insere linha
dw_Manut_Depto.insertRow (vll_NumLinhaAtual)
...
// insere linha
dw_Manut_Depto.deleteRow (vll_NumLinhaAtual)
...
vll_QtdLinhas = dw_Manut_Depto.retrieve()
...
// exporta arquivo
vli_Retorno = dw_Manut_Depto.saveAs (vls_NomeArquivoCompleto, vlsat_TipoArquivo, true)
choose case vli_Retorno
	case 1
...
vll_Retorno = dw_Manut_Depto.acceptText( )
...
if (dw_Manut_Depto.modifiedCount() = 0) and (dw_Manut_Depto.deletedCount() = 0) then
...
vll_Retorno = dw_Manut_Depto.update()
...
for i = 1 to vli_NumLinhas
	vls_Faixa = dw_Manut_Depto.getItemString (i, 'cf_faixa')
	dw_Manut_Depto.setItem (i, 'faixa', vls_Faixa)
next
...
// inicialização
vlb_Ok = true
vll_CodCargoLinha = dw_Manut_Depto.getItemNumber (ai_NumLinha, 'cod_cargo')
vli_QtdLinhas = dw_Manut_Depto.rowCount()
...
vli_NumLinhaDuplicada = dw_Manut_Depto.find (vls_ExpressaoBusca, ai_NumLinha + 1, vli_QtdLinhas) 
...
on w_manut_depto_manual.create
...
this.dw_manut_depto=create dw_manut_depto
...
on w_manut_depto_manual.destroy
...
destroy(this.dw_manut_depto)
end on

event open;dw_Manut_Depto.setTransObject (SQLCA)
